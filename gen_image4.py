...
print("base_resp:", result.get('base_resp'))

if 'data' in result and result['data'] and isinstance(result['data'], list) and len(result['data']) > 0 and 'base64' in result['data'][0]:
    ...
    print("Could not find base64 data in response. Full result structure:")
    print(json.dumps({k: str(type(v)) for k, v in result.items()}))
    if 'data' in result and isinstance(result['data'], list):
        print(f"data is list of len {len(result['data'])}")
    exit(1)
